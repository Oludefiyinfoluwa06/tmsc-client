export default function SystemSection() {
  ...
  return (
    <section className="w-full py-32 bg-linear-to-b from-white via-primary-50/30 to-white relative overflow-hidden">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 right-1/4 w-96 h-96 bg-primary-400/5 rounded-full blur-3xl"></div>
        <div className="absolute bottom-0 left-1/3 w-96 h-96 bg-primary-500/5 rounded-full blur-3xl"></div>
      </div>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="space-y-4">
            {points.map((point, index) => (
              <div key={index} className="flex gap-6 p-6 rounded-xl bg-white border border-gray-100 hover:border-primary-200 hover:shadow-lg transition-all duration-300 hover:bg-primary-50/30">
                <div className="shrink-0 flex items-center justify-center w-12 h-12 rounded-full bg-linear-to-br from-primary-500 to-primary-600 text-white font-bold text-lg mt-0.5">
                  {index + 1}
                </div>
