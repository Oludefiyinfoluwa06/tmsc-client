...
export default function MachineCategories({ categories }: MachineCategoriesProps) {
  return (
    <div id="machine-categories" className="mt-32 pt-32 border-t border-white/10">
      <div className="mb-16">
        <p className="text-primary-400 text-sm font-semibold uppercase tracking-widest mb-3">Equipment By Type</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {categories.map((category) => (
          <div key={category.id} className="group">
            <div className="absolute inset-0 bg-linear-to-br from-primary-600/20 to-blue-600/20 rounded-2xl blur-xl opacity-0 group-hover:opacity-100 transition-all duration-300"></div>
            <div className="relative">
              {/* Category Header */}
              <div className="bg-linear-to-br from-white/10 to-white/5 rounded-t-2xl p-6 border-t border-l border-r border-white/20 backdrop-blur-xl group-hover:border-primary-600/50 transition-all duration-300">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="text-white font-semibold text-lg mb-1">{category.name}</h3>
                    <p className="text-white/70 text-sm">{category.images.length} items</p>
                  </div>
                  <category.Icon className="text-primary-400" size={32} />
                </div>
              </div>

              {/* Category Images */}
              <div className="grid grid-cols-2 gap-3 p-4 bg-linear-to-br from-white/5 to-white/2 rounded-b-2xl border-b border-l border-r border-white/20 backdrop-blur-xl group-hover:border-primary-600/50 transition-all duration-300">
